...
You can install and load the latest version from [GitHub](https://github.com/) with:
...
df <- create_simulation_data(n_reps = 3) #Simulate 3 replicates per treatment
...
```df <- create_simulation_data(n_reps = 5, n_fish_per_tank = 60, days = 84) 
# View the first few rows of the data
head(df)
```
